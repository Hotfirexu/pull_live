...
func ParseSps(payload []byte, ctx *Context) error {
	...
	if err := parseSpsBasic(&br, &sps); err != nil {
		nazalog.Errorf("parseSpsBasic failed. err=%+v, payload=%s", err, hex.Dump(bytes.Prefix(payload, 128)))
		return err
	}
	...
	if err := parseSpsGamma(&br, &sps); err != nil {
		// 注意，这里不将错误返回给上层，因为可能是Beta自身解析的问题
		nazalog.Errorf("parseSpsGamma failed. err=%+v, payload=%s", err, hex.Dump(bytes.Prefix(payload, 128)))
	}
...
func TryParseSeqHeader(payload []byte) error {
	...
	var err error
	br := bits.NewBitReader(payload[5:])
...
func parseSpsBasic(br *bits.BitReader, sps *Sps) error {
...
func parseSpsGamma(br *bits.BitReader, sps *Sps) (err error) {
