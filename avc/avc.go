...
func IterateNaluAvcc(nals []byte, handler func(nal []byte)) error {
	...
	for {
		...
		if epos < len(nals) {
			...
			// length为0的直接过滤掉
			if length == 0 {
				nazalog.Warnf("avcc nalu length equal 0. nals=%s", bytes.Prefix(nals, 128))
				continue
			}
...
func parseSpsPpsListFromSeqHeaderWithoutMalloc(payload []byte) (spsList, ppsList [][]byte, err error) {
	...
	if !obytes.Equal(payload[:5], expected) {
		return nil, nil, base.ErrAvc
	}

	b := bits.NewBitReader(payload)
	// skip 10
