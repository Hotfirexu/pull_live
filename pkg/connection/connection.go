...
import (
	...
	"io"
	"net"
	"pull_live/pkg/atomic"
	"pull_live/pkg/unique"
	"sync"
	"time"
)
...
type Connection interface {
	...
	// Write
	// @return n 发送成功的大小
	// 如果设置了 Option.WriteChanSize 做异步发送，那么n恒等于len(b)
...
type StatAtomic struct {
	ReadBytesSum  atomic.Uint64
	WroteBytesSum atomic.Uint64
}
...
func New(conn net.Conn, modOption ...ModOption) Connection {
	c := new(connection)
	...
	nazalog.Debugf("[%s] lifecycle new connection. net.Conn=%p connection=%p",
		c.uniqueKey, conn, c)
...
func init() {
	uniqueGen = unique.NewSingleGenerator("CONN")
}
